<?xml version="1.0"?>
<app>

<category>
Window Managers
</category>

<name>
Compiz
</name>

<description>
   <am>OpenGL Window Manager with compositing</am>
   <ar>OpenGL Window Manager with compositing</ar>
   <bg>OpenGL Window Manager with compositing</bg>
   <ca>Gestor de finestres OpenGL amb composició</ca>
   <cs>OpenGL Window Manager with compositing</cs>
   <da>OpenGL-vindueshåndtering med compositing</da>
   <de>OpenGL Window Manager mit Compositing</de>
   <el>OpenGL Διαχειριστής παραθύρων με σύνθεση</el>
   <en>OpenGL Window Manager with compositing</en>
   <es>Gestor de ventanas OpenGL con compositor</es>
   <et>OpenGL Window Manager with compositing</et>
   <eu>OpenGL Window Manager with compositing</eu>
   <fa>OpenGL Window Manager with compositing</fa>
   <fi>OpenGL Window Manager with compositing</fi>
   <fr>Gestionnaire de fenêtre OpenGL avec compositeur</fr>
   <he_IL>OpenGL Window Manager with compositing</he_IL>
   <hi>OpenGL Window Manager with compositing</hi>
   <hr>OpenGL Window Manager with compositing</hr>
   <hu>OpenGL Window Manager with compositing</hu>
   <id>OpenGL Window Manager with compositing</id>
   <is>OpenGL Window Manager with compositing</is>
   <it>Window Manager OpenGL con compositing</it>
   <ja_JP>OpenGL Window Manager with compositing</ja_JP>
   <ja>OpenGL Window Manager with compositing</ja>
   <kk>OpenGL Window Manager with compositing</kk>
   <ko>OpenGL Window Manager with compositing</ko>
   <lt>OpenGL Window Manager with compositing</lt>
   <mk>OpenGL Window Manager with compositing</mk>
   <nb>OpenGL Window Manager with compositing</nb>
   <nl>OpenGL Scherm Beheer met compositing</nl>
   <pl>menedżer okien OpenGL z komponowaniem</pl>
   <pt_BR>OpenGL Window Manager with compositing</pt_BR>
   <pt>Gestor de Janelas em OpenGL com composição (compositing)</pt>
   <ro>OpenGL Window Manager with compositing</ro>
   <ru>Оконный менеджер на OpenGL с поддержкой композитинга</ru>
   <sk>OpenGL Window Manager with compositing</sk>
   <sl>OpenGL Window Manager with compositing</sl>
   <sq>OpenGL Window Manager with compositing</sq>
   <sr>OpenGL Window Manager with compositing</sr>
   <sv>OpenGL Fönsterhanterare med compositing</sv>
   <tr>OpenGL Window Manager with compositing</tr>
   <uk>віконний менеджер OpenGL  з композитінгом</uk>
   <zh_CN>OpenGL Window Manager with compositing</zh_CN>
   <zh_TW>OpenGL Window Manager with compositing</zh_TW>
</description>

<installable>
all
</installable>

<screenshot>none</screenshot>

<preinstall>

</preinstall>

<install_package_names>
compiz
</install_package_names>


<postinstall>

</postinstall>


<uninstall_package_names>
compiz
compiz-core
compiz-fusion-plugins-experimental
compiz-fusion-plugins-extra
compiz-fusion-plugins-main
compiz-gtk
compiz-plugins
compizconfig-settings-manager
emerald
emerald-themes
fusion-icon
libcompizconfig0
libemeraldengine0
python-compizconfig
simple-ccsm
</uninstall_package_names>
</app>
